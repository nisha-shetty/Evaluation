...
public class CryptLib {
	...
	public static final String md5(final String inputString) {
	    ...
	    try {
	    	...
	    	ErrorLogServices.logException(e, "md5");
	    }
	    return "";
	}
	 ...
	 */
		...
		int len = _encryptionKey.getBytes(STR_UTF8).length; // length of the key	provided
		
		if (_encryptionKey.getBytes(STR_UTF8).length > _key.length)
			len = _key.length;
		
		int ivlen = _initVector.getBytes(STR_UTF8).length;
		
		if(_initVector.getBytes(STR_UTF8).length > _iv.length)
			ivlen = _iv.length;
		
		System.arraycopy(_encryptionKey.getBytes(STR_UTF8), 0, _key, 0, len);
		System.arraycopy(_initVector.getBytes(STR_UTF8), 0, _iv, 0, ivlen);
		...
		if (_mode.equals(EncryptMode.ENCRYPT)) {
			...
			byte[] results = _cx.doFinal(_inputText.getBytes(STR_UTF8)); // Finish
										// multi-part
										// transformation
										// (encryption)
			Base64.Encoder encoder = Base64.getEncoder();
			_out= encoder.encodeToString(results);
										// output
		}
...
//					Base64.DEFAULT);
	...
	public static String SHA256 (String text, int length) throws NoSuchAlgorithmException, UnsupportedEncodingException {
	    ...
	    md.update(text.getBytes(STR_UTF8));
...
//			CryptLib obj=new CryptLib();
//			String Enc=obj.encrypt("Hithesh","DtnA%b+sF^>K{'nA,*m2'?yum,v4q^NeQmdBm(jQGQj>L,~}@n4qen!m]wBbR^u?",IV);
//			System.out.println("Enc= "+Enc);
			
			//String Dec=obj.decrypt(Enc,"KayPay",IV);
	    	...
	    	String key = CryptLib.SHA256("DtnA%b+sF^>K{'nA,*m2'?yum,v4q^NeQmdBm(jQGQj>L,~}@n4qen!m]wBbR^u?", 32); //32 bytes = 256 bit
	    	String ClientDecSMS = _crypt.decrypt("iQNxizn2roytnwIkdutpcw", key, "085f1a9c7a9b18fd");
	    	String iv = CryptLib.generateRandomIV(16); //16 bytes = 128 bit
	    	output = _crypt.encrypt(plainText, key, iv); //encrypt
	    	
	    	System.out.println("ClientDecSMS---> "+ClientDecSMS);
	    	System.out.println("iv---> "+iv);
